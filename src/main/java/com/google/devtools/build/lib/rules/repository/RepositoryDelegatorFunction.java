...
public final class RepositoryDelegatorFunction implements SkyFunction {
    ...
    try (SilentCloseable c =
      ...
      if (shouldUseVendorRepos(env, handler, rule)) {
        RepositoryDirectoryValue repositoryDirectoryValue =
            tryGettingValueUsingVendoredRepo(env, rule, repoRoot, repositoryName, handler, digestWriter);
        if (env.valuesMissing()) {
          return null;
        }
  ...
  private RepositoryDirectoryValue tryGettingValueUsingVendoredRepo(Environment env, Rule rule, Path repoRoot, RepositoryName repositoryName, RepositoryFunction handler,
    ...
    if(vendorRepoPath.exists()){
      ...
      // If our repo is up-to-date, or this is an offline build (--nofetch) then out-of-date
      // vendored repo is used with a warning
      if (isVendorRepoUpToDate || (!IS_VENDOR_COMMAND.get(env).booleanValue() && !isFetch.get())) {
        if((!IS_VENDOR_COMMAND.get(env).booleanValue() && !isFetch.get())) {
          env.getListener()
              .handle(
                  Event.warn(
                      rule.getLocation(),
                      String.format(
                          "Vendored repository '%s' is out-to-date and fetching is disabled."
                              + " Run build without the '--nofetch' option or to update it, run"
                              + " `bazel vendor`",
                          rule.getName())));
        }
        return setupOverride(vendorRepoPath.asFragment(), env, repoRoot,
